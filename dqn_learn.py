...
def dqn_learing(
    ...
    Q_target = q_func(in_channels=input_arg, num_actions=num_actions)
    ...
    for t in count():
        ...
        #####
        store_inx = replay_buffer.store_frame(last_obs)
        encoded_data = replay_buffer.encode_recent_observation()
        action = select_epilson_greedy_action(Q, encoded_data, t)
        last_obs, reward, done, info = env.step(action)
        replay_buffer.store_effect(store_inx, action, reward, done)
        ...
        if (t > learning_starts and
            ...
            obs_batch = Variable(torch.from_numpy(obs_batch).type(dtype) / 255.0, volatile=False)
            act_batch = Variable(torch.from_numpy(act_batch).long())
            rew_batch = Variable(torch.from_numpy(rew_batch))
            next_obs_batch = Variable(torch.from_numpy(next_obs_batch).type(dtype)/ 255.0, volatile=False)
            done_mask = torch.from_numpy(done_mask).unsqueeze(1)
            if USE_CUDA:
            ...
            #3.b
            # bellman error: r(s_t,a_t) + gamma*maxQ(s_t+1, a) - Q(s_t, a_t)
           
            Q_calcs = Q(obs_batch)
            next_Q = Q_target(next_obs_batch).detach()
            max_next = torch.max(next_Q, dim=1).values
            b_error = torch.zeros_like(Q_calcs)
            for index in range(len(obs_batch)-1):
                if done_mask[index]:
                    continue
                else:
                    b_error[index] = rew_batch[index] + gamma*next_Q[index] - Q_calcs[index][act_batch[index]]
            error = b_error.clamp(-1,1) * (-1.0)
            #3.c
            optimizer.zero_grad()
            
            Q_calcs.backward(b_error.data)
            optimizer.step()
            #3.d
            if t%target_update_freq==0:
                Q_target.load_state_dict(Q.state_dict())
                num_param_updates += 1
                torch.save(Q.state_dict(), "/DQN_model.dqn")
